...
get.PK <- function(ds1a, ds1b, ds2, ds3, lday, dfreq1, dfreq2, params.mod) {    
  ...
  for (i in 1:nrow(params.mod)){
    ...
    if(dose1b>0) dosing <- dosing %>%
      add.dosing(dose=dose1b,start.time=12,nbr.doses=1)
    
    dosing <- dosing %>%
      add.dosing(dose=dose2,start.time=24,nbr.doses=1)
    if(dfreq1==2) dosing <- dosing %>%
      add.dosing(dose=dose2,start.time=36,nbr.doses=1)
    ...
    if(dfreq2==1) dosing <- dosing %>%
      ...
      add.dosing(dose=dose3,start.time=48,dosing.interval=12,nbr.doses=(lday-2)*2)
...
sum.PKparams<-function(ds,start,end){
  sumPKparams<-data.frame(STAT=c("5th Pctl","Median","95th Pctl"),
                          ds %>% filter(intStart*24==start) %>%
                            group_by(NULL) %>% 
                            dplyr::summarize(Cmin=quantile(Cmin,p=c(.05,.5,.95)),
...
server <- function(input, output) {
  ...
  get.cp <- eventReactive(input$submit_mod, {   
    get.PK(ds1a(), ds1b(), ds2(), ds3(), lday(), dfreq1(), dfreq2(), sim.params())
  })
